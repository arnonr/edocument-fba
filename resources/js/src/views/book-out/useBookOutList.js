...
export default function useBookOutList() {
      ...
      .then((response) => {
        ...
        bookOutLists.value = bookOut.map((item) => {
          ...
          book.dateReturn = item.dateReturn
            ? dayjs(item.dateReturn).locale("th").format("DD MMM BB")
            : "";
          ...
          bookCode = bookNoArr[0].replace("บธ(1)", "");
          bookCode = bookCode.replace("บธ(2)", "");
          bookCode = bookCode.replace("บธ", "");
          bookCode = bookCode.replace("อว", "");
          bookCode = bookCode.replace("คำสั่ง", "");

          book.bookNo = bookCode + "/" + bookNoArr[1];
          // .substr(bookNoArr[0].length-3,3)
        ...
        if (showBtnAdmin != true) {
          bookOutLists.value = bookOutLists.value.filter((x) => {
            if (x.departmentID != null) {
              return x.departmentID == getUserData().department.id;
            } else {
              return true;
            }
          });
      ...
      .then((response) => {
        if (response.data.message == "success") {
          ...
          if (statusID == 2) {
            ...
            response.data.dateReceiveReal = response.data.dateReceive
              ? dayjs(response.data.dateReceive).format("YYYY-MM-DD")
              : "";
          ...
          if (statusID == 4) {
            ...
            response.data.dateReturnReal = response.data.dateReturn
              ? dayjs(response.data.dateReturn).format("YYYY-MM-DD")
              : "";

            items.value[found].dateReturn = response.data.dateReturn
              ? dayjs(response.data.dateReturn).locale("th").format("DD MMM BB")
              : "";
          }
  ...
  const onExportExcel = () => {
    ...
    let exportExcel = bookOutLists.value.filter((x) => {
      if (dayjs(x.dateSendReal).isBetween(exportDate.start, exportDate.end)) {
        return true;
      } else {
